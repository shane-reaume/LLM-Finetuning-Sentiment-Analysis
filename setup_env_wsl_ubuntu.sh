...
echo "Setting up virtual environment for LLM-Finetuning-Sentiment-Analysis..."

# Check if Python 3.13 is available
python3.13 --version | grep "3.13" > /dev/null || {
    echo "Python 3.13.x is required. Please install it before continuing with this command:"
    echo "sudo add-apt-repository ppa:deadsnakes/ppa && sudo apt update && sudo apt install python3.13 python3.13-venv python3.13-dev"
    exit 1
}
